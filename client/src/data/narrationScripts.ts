...
// ---------------------------------------------------------------------------
// Narration des 25 pages de sous-expertises (/services/:service/:expertise)
// Ces pages n'ont pas (encore) d'enregistrement audio dédié. Le texte est
// dérivé automatiquement de serviceExpertises.ts (résumé + explication),
// déjà rédigé avec soin — pas de duplication à maintenir à la main.
// En l'absence de fichier audio, PageAudioControl bascule automatiquement
...
const expertiseNarrationEntries: Record<
  string,
  { label: string; audioUrl: string | null; text: string }
> = Object.fromEntries(
  serviceExpertises.map((expertise) => {
    const path = `/services/${expertise.serviceSlug}/${expertise.slug}`;
    return [
      path,
      {
...
function getExpertiseNarrationScript(pathname: string) {
  return expertiseNarrationEntries[pathname] ?? null;
}

export type NarrationPath = keyof typeof narrationScripts;

export function getNarrationScript(pathname: string) {
  const staticScript = narrationScripts[pathname as NarrationPath];
  if (staticScript) return staticScript;

  const expertiseScript = getExpertiseNarrationScript(pathname);
  if (expertiseScript) return expertiseScript;
