...
export default function AdminLogin() {
  ...
  return <main className="grid min-h-screen place-items-center bg-[#0B1233] px-5 py-12">
    <section className="w-full max-w-md rounded-[2rem] border border-white/10 bg-white p-7 shadow-[0_32px_100px_rgba(0,0,0,.34)] sm:p-10">
      ...
      <p className="mt-3 text-sm leading-6 text-slate-600">Connecte-toi avec ton e-mail et ton mot de passe administrateur.</p>
      <form className="mt-7 space-y-5" onSubmit={submit}>
        ...
        <Button type="submit" className="w-full rounded-full bg-[#18B7E8] font-bold text-[#0B1233] hover:bg-[#08A3D6]" disabled={isSubmitting}><KeyRound size={16} />{isSubmitting ? "Connexion…" : "Accéder au back office"}</Button>
      </form>
    </section>
  </main>;
}
